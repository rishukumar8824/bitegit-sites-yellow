...
function showRejectReasonModal(defaultReason, title, btnText) {
  const _title = title || 'Reject KYC';
  const _btnText = btnText || 'Reject KYC';
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div style="background:#1a1d2e;border:1px solid rgba(255,255,255,0.1);border-radius:14px;padding:24px;width:100%;max-width:440px;box-shadow:0 20px 60px rgba(0,0,0,0.6);">
        <div style="font-size:15px;font-weight:700;color:#e2e8f0;margin-bottom:6px;">❌ ${_title}</div>
        <div style="font-size:12px;color:#848e9c;margin-bottom:14px;">Enter the rejection reason. This will be shown to the user.</div>
        <textarea id="rejectReasonInput" rows="3" style="width:100%;box-sizing:border-box;background:#0d0f1a;border:1px solid rgba(0,184,212,0.35);border-radius:8px;padding:10px 12px;font-size:13px;color:#e2e8f0;outline:none;resize:vertical;font-family:inherit;">${defaultReason||''}</textarea>
        <div style="display:flex;gap:10px;margin-top:14px;">
          ...
          <button id="rejectConfirmBtn" style="flex:1;padding:9px;border-radius:8px;border:none;background:linear-gradient(135deg,#f6465d,#c53030);color:#fff;font-size:13px;font-weight:700;cursor:pointer;">${_btnText}</button>
        </div>
      </div>`;
...
const DEPOSIT_REJECT_REASONS = [
  'Select a reason…',
  'Invalid or unverified TxID — transaction not found on blockchain.',
  'Payment proof is unclear, incomplete, or invalid.',
  'Deposit amount does not match the submitted proof.',
  'Network / chain mismatch — please resubmit on the correct network.',
  'Duplicate transaction — this TxID has already been processed.',
  'Suspicious transaction flagged by our compliance team.',
  'KYC verification required before deposit can be approved.',
  'Deposit temporarily on hold — please contact support.',
];

function showDepositRejectModal() {
  return new Promise((resolve) => {
    const overlay = document.createElement('div');
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;';
    overlay.innerHTML = `
      <div style="background:#1a1d2e;border:1px solid rgba(255,255,255,0.1);border-radius:14px;padding:24px;width:100%;max-width:440px;box-shadow:0 20px 60px rgba(0,0,0,0.6);">
        <div style="font-size:15px;font-weight:700;color:#e2e8f0;margin-bottom:6px;">❌ Reject Deposit</div>
        <div style="font-size:12px;color:#848e9c;margin-bottom:12px;">Select or enter a rejection reason. This will be shown to the user.</div>
        <select id="depositRejectSelect" style="width:100%;box-sizing:border-box;background:#0d0f1a;border:1px solid rgba(0,184,212,0.35);border-radius:8px;padding:9px 12px;font-size:13px;color:#e2e8f0;outline:none;margin-bottom:10px;cursor:pointer;">
          ${DEPOSIT_REJECT_REASONS.map((r, i) => `<option value="${i === 0 ? '' : r}">${r}</option>`).join('')}
        </select>
        <textarea id="depositRejectInput" rows="3" placeholder="Or type a custom reason…" style="width:100%;box-sizing:border-box;background:#0d0f1a;border:1px solid rgba(0,184,212,0.35);border-radius:8px;padding:10px 12px;font-size:13px;color:#e2e8f0;outline:none;resize:vertical;font-family:inherit;"></textarea>
        <div style="display:flex;gap:10px;margin-top:14px;">
          <button id="depRejectCancelBtn" style="flex:1;padding:9px;border-radius:8px;border:1px solid rgba(255,255,255,0.1);background:transparent;color:#848e9c;font-size:13px;font-weight:600;cursor:pointer;">Cancel</button>
          <button id="depRejectConfirmBtn" style="flex:1;padding:9px;border-radius:8px;border:none;background:linear-gradient(135deg,#f6465d,#c53030);color:#fff;font-size:13px;font-weight:700;cursor:pointer;">Reject Deposit</button>
        </div>
      </div>`;
    document.body.appendChild(overlay);
    const sel = overlay.querySelector('#depositRejectSelect');
    const ta  = overlay.querySelector('#depositRejectInput');
    sel.onchange = () => { if (sel.value) ta.value = sel.value; };
    overlay.querySelector('#depRejectCancelBtn').onclick = () => { document.body.removeChild(overlay); resolve(null); };
    overlay.querySelector('#depRejectConfirmBtn').onclick = () => {
      const val = (ta.value.trim() || sel.value || '').trim();
      document.body.removeChild(overlay);
      resolve(val || 'Deposit rejected by admin.');
    };
  });
}
...
async function apiRequest(path, options = {}, _retried = false) {
  ...
  try {
    ...
    if (!_retried) {
      _showRestartOverlay();
      _serverRestartDeadline = Date.now() + 90000;
      await _sleep(4000);
      _hideRestartOverlay();
  ...
  if (response.status === 401 || response.status === 503) {
    if (!_retried && !path.includes('/auth/refresh')) {
      ...
      // Refresh also failed — server may be restarting, wait and retry for up to 90s
      _showRestartOverlay();
      _serverRestartDeadline = Date.now() + 180000;
      let _brokeOn401 = false;
      while (Date.now() < _serverRestartDeadline) {
        ...
        try {
          ...
          if (retryResp.ok) {
            // Token valid — check if the admin service is ready yet
            try {
              const _svcChk = await fetch(`${API_BASE}${path}`, {
                credentials: 'include',
                headers: { 'Content-Type': 'application/json', ...(options.headers || {}) },
                ...options
              });
              if (_svcChk.status !== 503) {
                _hideRestartOverlay();
                return apiRequest(path, options, true);
              }
              // Service still initializing — keep waiting
            } catch (_) { /* keep waiting */ }
            continue;
          }
          // 401 = server is up but token is genuinely invalid — stop waiting, go to login
          if (retryResp.status === 401) { _brokeOn401 = true; break; }
        } catch (_) { /* server still down, keep waiting */ }
      }
      _hideRestartOverlay();
      // Token expired → send to login; timeout → reload so user retries after server starts
      if (_brokeOn401) { window.location.href = '/admin/login'; } else { window.location.reload(); }
...
function buildDisputeMsgBubble(msg, buyerLabel, sellerLabel) {
  ...
  if (rawSender.startsWith('admin') || rawSender === 'admin' || msg.sender === 'Support') {
    roleLabel = 'Support'; roleColor = '#00b8d4'; bubble = 'rgba(0,229,255,0.10)';
...
function tradeChatQuickReply(idx) {
  ...
  if (!item) return;
  if (item.autoSend) {
    _showQuickReplyConfirm(item.text, function() { _sendTradeChatDirect(item.text); });
  } else {
...
function _showQuickReplyConfirm(text, onConfirm) {
  var existing = document.getElementById('_qrConfirmOverlay');
  if (existing) existing.remove();
  var overlay = document.createElement('div');
  overlay.id = '_qrConfirmOverlay';
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9999;display:flex;align-items:center;justify-content:center;';
  overlay.innerHTML = '<div style="background:#1e293b;border:1px solid rgba(255,255,255,0.12);border-radius:14px;padding:24px;max-width:420px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,0.5);">'
    + '<p style="font-size:12px;font-weight:700;color:#94a3b8;letter-spacing:.6px;margin:0 0 10px;">CONFIRM & SEND</p>'
    + '<p style="font-size:13px;color:#e2e8f0;line-height:1.6;margin:0 0 20px;background:rgba(255,255,255,0.04);border-radius:8px;padding:12px;">' + escapeHtml(text) + '</p>'
    + '<div style="display:flex;gap:10px;">'
    + '<button id="_qrConfirmBtn" style="flex:1;background:#22c55e;color:#fff;border:none;border-radius:8px;padding:10px;font-size:13px;font-weight:600;cursor:pointer;">Send ✓</button>'
    + '<button id="_qrCancelBtn" style="flex:1;background:rgba(255,255,255,0.07);color:#94a3b8;border:1px solid rgba(255,255,255,0.1);border-radius:8px;padding:10px;font-size:13px;cursor:pointer;">Cancel</button>'
    + '</div></div>';
  document.body.appendChild(overlay);
  document.getElementById('_qrConfirmBtn').onclick = function() { overlay.remove(); onConfirm(); };
  document.getElementById('_qrCancelBtn').onclick = function() { overlay.remove(); };
  overlay.onclick = function(e) { if (e.target === overlay) overlay.remove(); };
}

async function _sendTradeChatDirect(message) {
  if (!_tradeChatOrderId || !message) return;
...
const ADMIN_QUICK_REPLIES = [
  // ── Pending Order Reasons ──────────────────────────────────────────────────
  { label: '🔐 Security Deposit', autoSend: true, text: 'Dear User, due to multiple order cancellations, a refundable security deposit of 100 USDT is required to continue trading. This deposit will be held for 24 hours and automatically released for withdrawal thereafter.' },
  { label: '⏳ Payment Pending', autoSend: true, text: 'Dear User, your payment has not been received yet. Please complete the payment within the remaining time or your order will be automatically cancelled.' },
  { label: '🔍 Under Review', autoSend: true, text: 'Dear User, your order is currently under review by our compliance team. Please wait while we verify your transaction details. We will update you shortly.' },
  { label: '⚠️ Order On Hold', autoSend: true, text: 'Dear User, your order has been placed on hold due to unusual activity detected on your account. Please contact support for further assistance.' },
  // ── Dispute Replies ────────────────────────────────────────────────────────
  ...
  return `
    <article class="list-item" style="border-color:rgba(245,158,11,.35);background:rgba(15,23,42,.72);">
      ...
      <div class="mt-3 flex flex-wrap gap-2">
        <button onclick="adminReleaseEscrow('${orderId}','buyer',this)"
        ...
        <button onclick="adminReleaseEscrow('${orderId}','seller',this)"
          style="background:#f59e0b;color:#000;font-size:12px;font-weight:700;border:none;border-radius:8px;padding:8px 16px;cursor:pointer;">
          ✅ Release → Seller
        </button>
...
async function adminReleaseEscrow(orderId, releaseTo, btn) {
  var label = releaseTo === 'seller' ? 'Release → Seller' : 'Release → Buyer';
  ...
  try {
    var res = await fetch('/api/admin/p2p/orders/' + encodeURIComponent(orderId) + '/admin-release', {
      method: 'POST', credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ releaseTo: releaseTo || 'buyer' })
    });
    var data = await res.json().catch(function(){ return {}; });
    if (!res.ok) throw new Error(data.message || 'HTTP ' + res.status);
    showMessage('✅ Escrow released to ' + (releaseTo === 'seller' ? 'seller' : 'buyer') + '. Dispute resolved.', 'success');
...
async function loadSupport() {
  ...
  list.innerHTML = tickets.map((ticket) => {
    ...
    const uName = ticket.username || ticket.name || '';
    const uEmail = ticket.email || ticket.userEmail || '';
    const userLabel = uName && uEmail ? `${uName} (${uEmail})` : uName || uEmail || (userId ? userId.replace('usr_','').slice(0,16) : 'Unknown User');
...
async function renderTicketChat(ticketId, silent = false) {
  try {
    ...
    if (nameEl) {
      const uName = ticket.username || ticket.name || '';
      const uEmail = ticket.email || ticket.userEmail || '';
      const userLabel = uName && uEmail ? `${uName} (${uEmail})` : uName || uEmail || ticket.userId || 'Unknown User';
      nameEl.textContent = userLabel;
      nameEl.title = userLabel;
    }

    // User online status
    const onlineEl = document.getElementById('chatUserOnline');
    if (onlineEl) {
      const lastSeen = ticket.userLastSeenAt ? new Date(ticket.userLastSeenAt).getTime() : 0;
      const isOnline = lastSeen && (Date.now() - lastSeen < 65000);
      onlineEl.innerHTML = isOnline
        ? '<span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#02c076;margin-right:4px;vertical-align:middle;"></span><span style="color:#02c076;font-size:11px;">Online</span>'
        : '<span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#555;margin-right:4px;vertical-align:middle;"></span><span style="color:#555;font-size:11px;">Offline</span>';
    }

    // User typing indicator
    const userTypingEl = document.getElementById('chatUserTyping');
    if (userTypingEl) {
      const typingTs = ticket.userTypingAt ? new Date(ticket.userTypingAt).getTime() : 0;
      userTypingEl.style.display = typingTs && (Date.now() - typingTs < 3500) ? '' : 'none';
    }
    ...
    if (chatMessages) {
      chatMessages.innerHTML = messages.length === 0
        ...
        : messages.map((msg) => {
            ...
            const ticketUserName = ticket.username || ticket.name || ticket.email || ticket.userId || 'User';
            const senderName = isAdmin ? 'Support' : ticketUserName;
...
let _adminTypingDebounce = null;
function sendAdminTypingPing() {
  const ticketId = state.support.activeTicketId;
  if (!ticketId) return;
  clearTimeout(_adminTypingDebounce);
  _adminTypingDebounce = setTimeout(() => {
    apiRequest(`/admin/support/ticket/${encodeURIComponent(ticketId)}/typing`, { method:'POST', body: JSON.stringify({}) }).catch(()=>{});
  }, 300);
}
...
async function handleDepositAction(event) {
  ...
  if (decision === 'REJECTED') {
    reason = await showDepositRejectModal();
    if (!reason) return;
  } else {
  ...
  try {
    ...
    showMessage(
      decision === 'APPROVED' ? 'Deposit approved. Balance credited to user.' : 'Deposit rejected. User will be notified with reason.',
...
async function loadUpOverview() {
  ...
  el.innerHTML = `
    <div style="background:var(--bg-card);border-radius:10px;border:1px solid var(--border);padding:14px 16px;margin-bottom:10px;">
      ...
      <div class="up-info-row"><span class="up-info-label">Email</span><span class="up-info-value"><input id="upFieldEmail" class="input-dark" type="email" value="${escapeHtml(user.email||'')}" style="width:100%;height:30px;padding:4px 10px;font-size:12px;" /></span></div>
      <div class="up-info-row"><span class="up-info-label">Full Name</span><span class="up-info-value"><input id="upFieldFullName" class="input-dark" type="text" value="${escapeHtml(user.fullName||'')}" placeholder="Not provided" style="width:100%;height:30px;padding:4px 10px;font-size:12px;" /></span></div>
      <div class="up-info-row"><span class="up-info-label">Mobile</span><span class="up-info-value"><input id="upFieldMobile" class="input-dark" type="text" value="${escapeHtml(user.mobile||'')}" placeholder="Not provided" style="width:100%;height:30px;padding:4px 10px;font-size:12px;" /></span></div>
      <div class="up-info-row"><span class="up-info-label">Address</span><span class="up-info-value"><input id="upFieldAddress" class="input-dark" type="text" value="${escapeHtml(user.address||'')}" placeholder="Not provided" style="width:100%;height:30px;padding:4px 10px;font-size:12px;" /></span></div>
      ...
      <div style="display:flex;gap:8px;margin-top:12px;">
        <button onclick="upSaveProfile()" style="flex:1;padding:9px;background:var(--accent);border:none;border-radius:8px;color:#000;font-size:12px;font-weight:700;cursor:pointer;">💾 Save Changes</button>
        <button onclick="loadUpOverview()" style="flex:1;padding:9px;background:var(--bg-card2);border:1px solid var(--border);border-radius:8px;color:var(--text-2);font-size:12px;font-weight:600;cursor:pointer;">✕ Discard</button>
      </div>
    </div>
...
async function upAction(action) {
  ...
  try {
    if (action === 'freeze') {
      ...
      const email    = document.getElementById('upEditEmail').value.trim();
      const fullName = document.getElementById('upEditFullName').value.trim();
      const mobile   = document.getElementById('upEditMobile').value.trim();
      const address  = document.getElementById('upEditAddress').value.trim();
      if (!email && !fullName && !mobile && !address) return showMessage('Fill at least one field to update.', 'error');
      const patch = {};
      if (email)    patch.email    = email;
      if (fullName) patch.fullName = fullName;
      if (mobile)   patch.mobile   = mobile;
      if (address)  patch.address  = address;
      const r2 = await fetch(`/api/admin/users/${encodeURIComponent(_upUserId)}/profile`, {
        method: 'PUT', credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(patch)
      });
      const d2 = await r2.json();
      if (!d2.success) throw new Error(d2.message || 'Failed to update profile');
      showMessage('Profile updated successfully.', 'success');
      ['upEditEmail','upEditFullName','upEditMobile','upEditAddress'].forEach(id => {
...
async function upSaveProfile() {
  if (!_upUserId) return;
  const email    = (document.getElementById('upFieldEmail')?.value    || '').trim();
  const fullName = (document.getElementById('upFieldFullName')?.value || '').trim();
  const mobile   = (document.getElementById('upFieldMobile')?.value   || '').trim();
  const address  = (document.getElementById('upFieldAddress')?.value  || '').trim();
  const patch = {};
  if (email)    patch.email    = email;
  if (fullName) patch.fullName = fullName;
  if (mobile)   patch.mobile   = mobile;
  if (address)  patch.address  = address;
  if (!Object.keys(patch).length) { showMessage('Edit at least one field first.', 'error'); return; }
  const saveBtn = document.querySelector('button[onclick="upSaveProfile()"]');
  if (saveBtn) { saveBtn.disabled = true; saveBtn.textContent = 'Saving…'; }
  try {
    const r = await fetch('/api/admin/users/' + encodeURIComponent(_upUserId) + '/profile', {
      method: 'PUT', credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(patch)
    });
    ...
    <div style="border-bottom:1px solid rgba(255,255,255,0.06);">
      ...
      <div id="${detailId}" style="display:none;padding:0 16px 14px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
          <button onclick="wdAction('${escapeHtml(id)}','APPROVED',this,'wdAddr_${detailId}')"
            style="background:#02c076;color:#fff;border:none;border-radius:8px;padding:10px 6px;font-size:11px;font-weight:800;cursor:pointer;">✓ Approve</button>
          <button onclick="wdAction('${escapeHtml(id)}','REJECTED',this,'wdAddr_${detailId}')"
            style="background:#f6465d;color:#fff;border:none;border-radius:8px;padding:10px 6px;font-size:11px;font-weight:800;cursor:pointer;">✕ Reject</button>
          <button onclick="wdSaveAddress('${escapeHtml(id)}','wdAddr_${detailId}',this)"
            style="background:#f0b90b;color:#000;border:none;border-radius:8px;padding:10px 6px;font-size:11px;font-weight:800;cursor:pointer;">✎ Fix Addr</button>
        </div>
      </div>
    </div>`;
  }).join('');
...
async function wdSaveAddress(withdrawalId, addrInputId, btn) {
  const inp = document.getElementById(addrInputId);
  if (!inp) return;
...
async function wdAction(withdrawalId, decision, btn, addrInputId) {
  btn.disabled = true;
  btn.textContent = '…';
  try {
    let reason = decision === 'APPROVED' ? 'Approved by admin' : '';
    if (decision === 'REJECTED') {
      reason = await showRejectReasonModal('', 'Reject Withdrawal', 'Reject Withdrawal');
...
function switchEmailTab(tab) {
  const tabs = ['send', 'inbox', 'templates'];
  tabs.forEach(t => {
    const panel = document.getElementById('email' + t.charAt(0).toUpperCase() + t.slice(1) + 'Tab');
    const btn = document.getElementById('emailTab' + t.charAt(0).toUpperCase() + t.slice(1));
...
function handleEmailUserSearch(val) {
  clearTimeout(emailSearchTimeout);
  const dd = document.getElementById('emailUserDropdown');
  if (!dd) return;
  if (val.length < 3) { dd.style.display = 'none'; return; }
  emailSearchTimeout = setTimeout(async () => {
    try {
      const res = await fetch(`${API_BASE}/email/users/search?q=${encodeURIComponent(val)}`);
      const data = await res.json();
      if (!data.users || data.users.length === 0) { dd.style.display = 'none'; return; }
      dd.innerHTML = data.users.map(u => `
        <div onclick="selectEmailUser('${escapeHtml(u.email)}','${escapeHtml(u.username||u.email)}')"
          style="padding:10px 14px;cursor:pointer;border-bottom:1px solid rgba(255,255,255,0.05);font-size:13px;"
          onmouseover="this.style.background='rgba(240,185,11,0.1)'" onmouseout="this.style.background=''">
          <div style="color:#fff;font-weight:600;">${escapeHtml(u.username || u.email)}</div>
          <div style="color:rgba(255,255,255,0.4);font-size:11px;">${escapeHtml(u.email)}</div>
        </div>`).join('');
...
function selectEmailUser(email, name) {
  emailSelectedUser = { email, name };
  const inp = document.getElementById('emailToInput');
...
function updateEmailPreview() {
  ...
  if (subEl) subEl.textContent = subject || 'Subject will appear here';
  if (bodyEl) bodyEl.textContent = body || 'Your message body will appear here...';
}
  ...
  try {
    ...
    if (sel) {
      sel.innerHTML = '<option value="">— Select a template —</option>' +
        emailTemplatesCache.map(t => `<option value="${escapeHtml(t._id)}">${escapeHtml(t.name)}</option>`).join('');
    }
    // Update templates list in tab
    const listEl = document.getElementById('templatesList');
    if (listEl) {
      if (emailTemplatesCache.length === 0) {
        listEl.innerHTML = '<div style="color:rgba(255,255,255,0.3);font-size:13px;text-align:center;padding:24px;">No templates yet. Add one →</div>';
      } else {
        listEl.innerHTML = emailTemplatesCache.map(t => `
          <div style="padding:12px 14px;background:rgba(255,255,255,0.04);border-radius:8px;border:1px solid rgba(255,255,255,0.08);">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              <div>
                <div style="font-size:13px;font-weight:600;color:#fff;">${escapeHtml(t.name)}</div>
                <div style="font-size:11px;color:rgba(255,255,255,0.4);margin-top:2px;">${escapeHtml(t.subject)}</div>
              </div>
              <button onclick="deleteEmailTemplate('${t._id}')"
                style="background:rgba(246,70,93,0.15);border:1px solid rgba(246,70,93,0.3);color:#f6465d;border-radius:6px;padding:4px 10px;font-size:11px;cursor:pointer;">Delete</button>
            </div>
          </div>`).join('');
      }
    }
...
function applyEmailTemplate(id) {
  if (!id) return;
  const tpl = emailTemplatesCache.find(t => t._id === id);
  if (!tpl) return;
  const subj = document.getElementById('emailSubjectInput');
  const body = document.getElementById('emailBodyInput');
...
async function sendAdminEmail() {
  ...
  if (!toEmail || !subject || !bodyText) {
    if (statusEl) { statusEl.textContent = '⚠ Please fill all fields.'; statusEl.style.color = '#f6465d'; }
    return;
  }
  if (statusEl) { statusEl.textContent = 'Sending...'; statusEl.style.color = 'rgba(255,255,255,0.5)'; }
  try {
    const res = await fetch(`${API_BASE}/email/send`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ toEmail, toName: emailSelectedUser?.name || '', subject, body: bodyText })
    });
    const data = await res.json();
    if (res.ok) {
      if (statusEl) { statusEl.textContent = '✓ Email sent successfully!'; statusEl.style.color = '#22c55e'; }
      if (toInput) toInput.value = '';
      if (subj) subj.value = '';
      if (body) body.value = '';
      emailSelectedUser = null;
      updateEmailPreview();
    } else {
      if (statusEl) { statusEl.textContent = '✗ ' + (data.message || 'Failed to send.'); statusEl.style.color = '#f6465d'; }
    }
  } catch(e) {
    if (statusEl) { statusEl.textContent = '✗ Network error.'; statusEl.style.color = '#f6465d'; }
  }
}

async function loadEmailInbox() {
  const listEl = document.getElementById('emailInboxList');
  if (!listEl) return;
  listEl.innerHTML = '<div style="padding:32px 16px;text-align:center;color:rgba(255,255,255,0.3);font-size:13px;">Loading...</div>';
  try {
    const res = await fetch(`${API_BASE}/email/inbox`);
    const data = await res.json();
...
async function sendInboxReply(id) {
  const bodyInput = document.getElementById('replyBody_' + id);
  const statusEl = document.getElementById('replyStatus_' + id);
  const replyBody = (bodyInput || {}).value || '';
  if (!replyBody.trim()) { if (statusEl) { statusEl.textContent = 'Write a reply first.'; statusEl.style.color = '#f6465d'; } return; }
  if (statusEl) { statusEl.textContent = 'Sending...'; statusEl.style.color = 'rgba(255,255,255,0.5)'; }
  try {
    const res = await fetch(`${API_BASE}/email/inbox/${id}/reply`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
      ...
      await openInboxMessage(id);
    } else {
      if (statusEl) { statusEl.textContent = '✗ ' + (data.message || 'Failed'); statusEl.style.color = '#f6465d'; }
    }
  } catch(e) {
    if (statusEl) { statusEl.textContent = '✗ Network error.'; statusEl.style.color = '#f6465d'; }
  }
...
async function saveNewTemplate() {
  ...
  const subject = (document.getElementById('newTplSubject') || {}).value || '';
  const body = (document.getElementById('newTplBody') || {}).value || '';
  const statusEl = document.getElementById('tplSaveStatus');
  if (!name || !subject || !body) {
    if (statusEl) { statusEl.textContent = '⚠ Fill all fields.'; statusEl.style.color = '#f6465d'; }
    return;
  }
  try {
    const res = await fetch(`${API_BASE}/email/templates`, {
    ...
    const data = await res.json();
    if (res.ok) {
      if (statusEl) { statusEl.textContent = '✓ Template saved!'; statusEl.style.color = '#22c55e'; }
