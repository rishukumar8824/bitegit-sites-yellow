  ...
  async function createEscrowOrder(orderInput) {
    ...
    try {
      ...
      const result = await (async () => {
        ...
        const sellerWallet = await wallets.findOne({ userId: sellerUserId });
        ...
        if (adUpdate.modifiedCount !== 1) {
          throw createAppError('AD_STATE_RACE', 'Ad balance changed while creating order. Please try again.', 409);
        }
