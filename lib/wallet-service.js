...
function createWalletService(collections, mongoClient, options = {}) {
  ...
  async function createEscrowOrder(orderInput) {
    ...
    try {
      // Non-transactional path (works on standalone MongoDB / no replica set).
      // Safety is maintained via optimistic locking: the availableAmount update
      // only succeeds when the value hasn't changed (modifiedCount guard).
      const result = await (async () => {
        const now = Date.now();

        // Simple lookup — controller already did full validation before calling us.
        // We only need the current availableAmount and _id for the optimistic lock update.
        ...
        // Skip seller locked-balance check — merchant deposit is locked at ad level, not wallet level.
        // (Wallet locked balance may not reflect ad-locked funds depending on setup.)

        const nextAvailable = toAmount(currentAvailable - escrowAmount);
        // Optimistic lock: match by _id + status only (availableAmount may not be a stored field on older ads)
        const adUpdate = await p2pOffers.updateOne(
          { _id: matchedOffer._id, status: 'ACTIVE' },
          {
            $set: {
