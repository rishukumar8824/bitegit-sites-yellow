...
} = require('./p2p-order-state');

const CANCELLABLE_ORDER_STATUSES = CREATED_LIKE_STATUSES;
...
function createOrderActionActor(actor = {}, fallbackRole = '') {
  return {
    id: String(actor?.id || actor?.userId || '').trim(),
    username: String(actor?.username || actor?.name || '').trim(),
    role: String(actor?.role || fallbackRole).trim(),
    email: String(actor?.email || '').trim(),
    isSystem: actor?.isSystem === true
  };
}
...
function createWalletService(collections, mongoClient, options = {}) {
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          // Atlas free tier or standalone — run without transaction
          payload = await work(null);
        } else {
          throw txErr;
        }
      }
  ...
  async function findBuyerActiveOrder(session, buyerUserId) {
    const normalizedBuyerUserId = ensureUserId(buyerUserId, 'buyerUserId');
    return p2pOrders.findOne(
      {
        status: { $in: ACTIVE_ORDER_STATUSES },
        $or: [{ buyerUserId: normalizedBuyerUserId }, { buyerId: normalizedBuyerUserId }]
      },
      {
        session,
        sort: { updatedAt: -1 }
  ...
  function createExistingActiveOrderError(order) {
    ...
    if (order) {
      error.existingOrder = order;
    }
    return error;
  }

  async function reserveBuyerActiveOrderSlot({ session, buyerUserId, buyerUsername, orderId }) {
    const normalizedBuyerUserId = ensureUserId(buyerUserId, 'buyerUserId');
    const slotOrderId = String(orderId || '').trim();
    const slotUsername = String(buyerUsername || normalizedBuyerUserId).trim();
    const now = new Date();

    const initialClaim = await wallets.updateOne(
      {
        userId: normalizedBuyerUserId,
        $or: [{ activeP2POrderId: { $exists: false } }, { activeP2POrderId: null }, { activeP2POrderId: '' }]
      },
      {
        $set: {
          username: slotUsername,
          activeP2POrderId: slotOrderId,
          activeP2POrderClaimedAt: now,
          updatedAt: now
        }
      },
      { session }
    ...
    if (initialClaim.modifiedCount === 1) {
      return;
    }

    const walletDoc = await wallets.findOne({ userId: normalizedBuyerUserId }, { session });
    const walletSlotOrderId = String(walletDoc?.activeP2POrderId || '').trim();
    let existingOrder = null;

    if (walletSlotOrderId) {
      existingOrder = await p2pOrders.findOne({ id: walletSlotOrderId }, { session });
      if (existingOrder && isActiveOrderStatus(existingOrder.status)) {
        throw createExistingActiveOrderError(existingOrder);
      }
    }

    existingOrder = await findBuyerActiveOrder(session, normalizedBuyerUserId);
    if (existingOrder) {
      if (walletDoc && String(walletDoc.activeP2POrderId || '').trim() !== String(existingOrder.id || '').trim()) {
        await wallets.updateOne(
          { _id: walletDoc._id },
          {
            $set: {
              activeP2POrderId: String(existingOrder.id || '').trim(),
              activeP2POrderClaimedAt: now,
              updatedAt: now
            }
          },
    ...
    if (!walletDoc) {
      throw createAppError('BUYER_WALLET_NOT_FOUND', 'Buyer wallet not found.', 404);
    }

    const recovered = await wallets.updateOne(
      {
        _id: walletDoc._id,
        activeP2POrderId: walletDoc.activeP2POrderId
      },
      {
        $set: {
          username: slotUsername,
    ...
    if (recovered.modifiedCount !== 1) {
      throw createAppError('ACTIVE_ORDER_SLOT_CONFLICT', 'An active order is already being created. Please retry.', 409);
    }
  }

  async function clearBuyerActiveOrderSlot({ session, buyerUserId, orderId, updatedAt = Date.now() }) {
    const normalizedBuyerUserId = ensureUserId(buyerUserId, 'buyerUserId');
    const slotOrderId = String(orderId || '').trim();
    await wallets.updateOne(
      {
      ...
      },
      {
        $set: {
          ...
          activeP2POrderReleasedAt: new Date(updatedAt),
  ...
  async function createEscrowOrder(orderInput) {
    ...
    try {
      const result = await runInTransaction(async (session) => {
        ...
        await reserveBuyerActiveOrderSlot({
          session,
          buyerUserId,
          buyerUsername,
        ...
        // Funds are already escrow-locked at ad creation time.
        // During order creation we only reserve from ad.availableAmount.
        const sellerWallet = await wallets.findOne({ userId: sellerUserId }, { session });
        const sellerLocked = resolveLockedBalance(sellerWallet || {});
        if (sellerLocked < escrowAmount) {
          throw createAppError(
            'INSUFFICIENT_ESCROW_LOCKED',
            'Seller escrow balance is insufficient for this order.',
            409
          );
        }
        ...
        const payWindow = matchedOffer.payWindow || 15;
        const orderDoc = {
          ...orderInput,
          adId,
          offerId: adId,
          ...
          chatReady: true,
          chatReadyAt: now,
          statusHistory: Array.isArray(orderInput.statusHistory) && orderInput.statusHistory.length
            ? orderInput.statusHistory
            : [
                createOrderStatusHistoryEntry({
                  status: ORDER_STATUS.CREATED,
                  actor: {
          ...
          messages: [
            {
              ...
              sender: 'System',
              role: 'buyer',
              text: `You have a pending order. Please complete the payment within ${payWindow} minutes or your order will be automatically canceled.`,
              createdAt: now
            },
            {
              id: `msg_${now}_sys_s`,
              sender: 'System',
              role: 'seller',
              text: `A buyer has placed an order with you. Please wait for the buyer to complete the payment. You will be notified once payment is confirmed.`,
              createdAt: now
            }
          ]
        };
      ...
      if (error?.code === 11000) {
        const existingOrder = await p2pOrders.findOne(
          {
            status: { $in: ACTIVE_ORDER_STATUSES },
            $or: [
              { buyerUserId: String(orderInput?.buyerUserId || '').trim() },
              { buyerId: String(orderInput?.buyerUserId || '').trim() }
            ]
          },
          { sort: { updatedAt: -1 } }
        );
        ...
        throw createAppError('ORDER_CREATE_FAILED', 'Order creation failed. Please try again.', 409);
      }
      throw error;
    }
  }
  ...
  async function markOrderPaid(orderId, actor) {
    ...
    if (normalizeOrderStatus(existingOrder.status) === ORDER_STATUS.PAYMENT_SENT) {
    ...
    if (!transition.allowed) {
      throw createAppError(transition.code, transition.message, transition.code === 'FORBIDDEN' ? 403 : 409);
    }
    ...
    const statusResult = await p2pOrders.updateOne(
      {
        id: orderId,
        status: { $in: CREATED_LIKE_STATUSES }
      },
      {
        $set: {
          status: ORDER_STATUS.PAYMENT_SENT,
          updatedAt: now,
          paymentSentAt: now
        },
        $push: {
            ...
            $each: [
              {
                ...
                sender: 'System',
                role: 'buyer',
                text: `You've marked the order as "Paid". Please wait for the seller to verify the payment and release the assets.`,
                createdAt: now
              },
              {
                id: `msg_${now}_paid_s`,
                sender: 'System',
                role: 'seller',
                text: `The buyer has marked the order as "Paid". Please verify the payment in your account and release the assets to complete the order.`,
                createdAt: now
              }
            ]
          },
          statusHistory: createOrderStatusHistoryEntry({
            status: ORDER_STATUS.PAYMENT_SENT,
            actor: createOrderActionActor(actor, 'buyer'),
  ...
  async function setOrderDisputed(orderId, actor) {
    ...
    if (normalizeOrderStatus(order.status) === ORDER_STATUS.DISPUTED) {
      return order;
    }
    const transition = canTransitionOrder({
      order,
    ...
    await p2pOrders.updateOne(
      { id: orderId },
      {
        ...
        $push: {
          ...
          statusHistory: createOrderStatusHistoryEntry({
            status: ORDER_STATUS.DISPUTED,
            actor: createOrderActionActor(actor, transition.actorRole || ''),
  ...
  async function releaseOrder(orderId, actor) {
    ...
    try {
      const result = await runInTransaction(async (session) => {
        ...
        if (normalizeOrderStatus(order.status) === ORDER_STATUS.COMPLETED) {
          return order;
        }
        const transition = canTransitionOrder({
          order,
        ...
        const statusUpdate = await p2pOrders.updateOne(
          {
            id: orderId,
            status: { $in: PAYMENT_SENT_LIKE_STATUSES }
          },
          {
            $set: {
            ...
            $push: {
                ...
                $each: [
                  {
                    ...
                    sender: 'System',
                    role: 'buyer',
                    text: `The seller has released the funds. ${feeAmount > 0 ? `Fee deducted: ${feeAmount} ${String(order.asset || 'USDT').toUpperCase()}. ` : ''}Your USDT has been credited to your wallet.`,
                    createdAt: now
                  },
                  {
                    id: `msg_${now}_rel_s`,
                    sender: 'System',
                    role: 'seller',
                    text: `You've successfully released the funds. ${feeAmount > 0 ? `Fee deducted: ${feeAmount} ${String(order.asset || 'USDT').toUpperCase()}. ` : ''}The order is now complete.`,
                    createdAt: now
                  }
                ]
              },
              statusHistory: createOrderStatusHistoryEntry({
                status: ORDER_STATUS.COMPLETED,
                actor: createOrderActionActor(actor, 'seller'),
        ...
        await clearBuyerActiveOrderSlot({
          session,
      ...
      // Update reputation for both seller and buyer
      try {
        const releaseTimeMs = result && result.paymentSentAt ? (result.completedAt || Date.now()) - result.paymentSentAt : 0;
        const sellerUserId = result && result.sellerUserId;
        const buyerUserId = result && result.buyerUserId;
        if (sellerUserId) await p2pCredentials.updateOne({ userId: sellerUserId }, { $inc: { rep_completedOrders: 1, rep_totalReleasedCount: 1, rep_totalReleaseTimeMs: releaseTimeMs } });
        if (buyerUserId) await p2pCredentials.updateOne({ userId: buyerUserId }, { $inc: { rep_completedOrders: 1 } });
      } catch (_repErr) { /* non-critical */ }
      return result;
    } catch (error) {
      await logFailedBalanceAttempt(
        'release_order',
  ...
  async function cancelOrder(orderId, actor, status = 'CANCELLED') {
    const targetStatus = status === ORDER_STATUS.EXPIRED ? ORDER_STATUS.EXPIRED : ORDER_STATUS.CANCELLED;
    const actorId = String(actor?.id || actor?.userId || '').trim();
    ...
    try {
      const result = await runInTransaction(async (session) => {
        ...
        if (normalizeOrderStatus(order.status) === targetStatus) {
          return order;
        }
        const transition = canTransitionOrder({
          order,
          action: targetStatus === ORDER_STATUS.EXPIRED ? 'expire' : 'cancel',
          actor,
          now: Date.now()
        });
        if (!transition.allowed) {
        ...
        await p2pOrders.updateOne(
          {
            id: orderId,
            status: { $in: targetStatus === ORDER_STATUS.EXPIRED ? EXPIRABLE_ORDER_STATUSES : CANCELLABLE_ORDER_STATUSES }
          },
          {
            $set: {
              status: targetStatus,
              updatedAt: now,
              cancelledAt: targetStatus === ORDER_STATUS.CANCELLED ? now : order.cancelledAt || null,
              expiredAt: targetStatus === ORDER_STATUS.EXPIRED ? now : order.expiredAt || null
            },
            $push: {
              messages: {
                id: `msg_${now}_${Math.floor(Math.random() * 1000)}`,
                sender: actorName,
                text:
                  targetStatus === ORDER_STATUS.EXPIRED
                    ? 'Order cancelled automatically because the payment time expired. Reserved ad liquidity was restored.'
                    : `${actorName} cancelled order. Reserved ad liquidity was restored.`,
                createdAt: now
              },
              statusHistory: createOrderStatusHistoryEntry({
                status: targetStatus,
                actor: createOrderActionActor(actor, isSystem ? 'system' : transition.actorRole || ''),
                reason: targetStatus === ORDER_STATUS.EXPIRED ? 'payment_window_expired' : 'order_cancelled',
                at: now,
                metadata: {
                  orderId: String(orderId || '').trim()
                }
      ...
      // Track cancellation in reputation (only real user cancels, not system/expiry)
      try {
        if (!isSystem && targetStatus === ORDER_STATUS.CANCELLED && actorId) {
          await p2pCredentials.updateOne({ userId: actorId }, { $inc: { rep_cancelledOrders: 1 } });
        }
      } catch (_repErr) { /* non-critical */ }
      return result;
    } catch (error) {
  ...
  async function expireOrders(nowMs = Date.now()) {
    ...
    for (const order of candidates) {
      try {
        const cancelledDoc = await cancelOrder(
          order.id,
          {
            userId: 'system',
            username: 'System',
        ...
        if (cancelledDoc) {
          expiredOrders.push({
            id: cancelledDoc.id || order.id,
            buyerUserId: cancelledDoc.buyerUserId || cancelledDoc.buyerId,
            sellerUserId: cancelledDoc.sellerUserId || cancelledDoc.sellerId
          });
        }
      } catch (error) {
    ...
    // Auto-deactivate offers where availableAmount is 0 or seller has insufficient wallet balance
    try {
      const activeOffers = await p2pOffers
        .find({ status: 'ACTIVE', createdByUserId: { $exists: true, $ne: null } })
        .project({ id: 1, createdByUserId: 1, minLimit: 1, availableAmount: 1 })
        .limit(200)
        .toArray();

      for (const offer of activeOffers) {
        const available = Number(offer.availableAmount || 0);
        if (available <= 0) {
          await p2pOffers.updateOne({ id: offer.id }, { $set: { status: 'PAUSED', updatedAt: Date.now() } });
          continue;
        }
        // Check wallet balance — pause if available balance < offer minLimit (in asset units approx)
        const wallet = await wallets.findOne({ userId: offer.createdByUserId });
        const walletAvailable = Number(wallet?.availableBalance ?? wallet?.available ?? 0);
        if (walletAvailable <= 0) {
          await p2pOffers.updateOne({ id: offer.id }, { $set: { status: 'PAUSED', updatedAt: Date.now() } });
        }
      }
    } catch (_) { /* auto-deactivate is non-critical */ }

    return { expired, expiredOrders };
  }
