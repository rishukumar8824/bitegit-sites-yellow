...
function createWalletService(collections, mongoClient, options = {}) {
  ...
  async function createEscrowOrder(orderInput) {
    ...
    // Only block if the seller has a LIVE (non-expired) active order.
    // Expired CREATED orders are treated as effectively cancelled — they must
    // not prevent new buyers from placing orders.
    const preNow = Date.now();
    const existingActive = await p2pOrders.findOne({
      sellerUserId,
      status: { $in: ACTIVE_ORDER_STATUSES },
      $or: [{ expiresAt: { $gt: preNow } }, { expiresAt: { $exists: false } }]
    });
    if (existingActive) {
      throw createAppError(
