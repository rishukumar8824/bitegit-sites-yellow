...
// Returns display name: if username is set (not email-like), use it; else mask email
function safeDisplayName(username, email) {
  const u = String(username || '').trim();
  const e = String(email || '').trim();
  // If username looks like an email (has @), treat it as no-username case
  if (u && !u.includes('@')) return u;
  // No username or username is email — mask the email: abc***@gmail.com
  const src = e || u;
...
function createWalletService(collections, mongoClient, options = {}) {
  ...
  async function createEscrowAd({ actor, offerId, payload }) {
    ...
    const actorEmail = String(actor?.email || '')
      .trim()
      .toLowerCase();
    const actorUsername = safeDisplayName(actor?.username, actorEmail);
    if (!actorEmail) {
  ...
  async function releaseOrder(orderId, actor) {
    ...
    try {
      ...
      const result = await (async () => {
        ...
        // Check seller wallet — use locked first, available as fallback
        const sellerWalletDoc = await wallets.findOne({ userId: order.sellerUserId }) || {};
        const sellerLocked    = Math.max(0, Number(sellerWalletDoc.p2pLocked || sellerWalletDoc.lockedBalance || 0));
        const fromLocked      = Math.min(sellerLocked, escrowAmount);
        const fromAvailable   = escrowAmount - fromLocked;

        await applyWalletDeltaInSession({
          session: null,
          userId: order.sellerUserId,
          username: order.sellerUsername,
          deltaAvailable: -fromAvailable,
          deltaLocked: -fromLocked,
          amount: escrowAmount,
          type: 'trade_sell',
    ...
    try {
      const processed = await runInTransaction(async (session) => {
        ...
        if (targetStatus === 'approved') {
          ...
          // Deduct locked balance immediately on approval — crypto is being sent
          await applyWalletDeltaInSession({
            session,
            ...
            type: 'withdrawal',
            currency,
            referenceId: `${normalizedRequestId}_approved`,
            metadata: {
              stage: 'withdrawal_approved_deduct',
              actorId,
              actorName,
