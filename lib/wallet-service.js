    ...
    try {
      const result = await (async () => {
        ...
        const adUpdate = await p2pOffers.updateOne(
          ...
          { $set: { availableAmount: nextAvailable, available: nextAvailable, updatedAt: new Date(now) } }
        );
      ...
      const processed = await runInTransaction(async (session) => {
        ...
        if (targetStatus === 'approved') {
          ...
          await applyWalletDeltaInSession({
            ...
            type: 'withdrawal',
            currency,
            referenceId: `${normalizedRequestId}_approve`,
            metadata: {
              stage: 'withdrawal_approved_finalize',
              actorId,
              actorName,
          ...
          if (currentStatus !== 'pending') {
            throw createAppError('INVALID_WITHDRAWAL_STATE', 'Only pending withdrawals can be rejected.');
          }
          await applyWalletDeltaInSession({
            session,
            userId: request.userId,
