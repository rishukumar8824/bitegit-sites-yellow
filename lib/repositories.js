...
async function verifyPassword(password, storedHash) {
  ...
  if (raw.startsWith('v2:')) {
    // v2 format: "v2:{N}:{salt}:{digest}" — handle hashes from brief v2 deployment
  ...
  const check = await new Promise((resolve, reject) =>
    crypto.scrypt(password, salt, 64, { N }, (err, key) => err ? reject(err) : resolve(key))
  );
  const digestBuffer = Buffer.from(digest, 'hex');
  if (digestBuffer.length !== check.length) return false;
...
function createRepositories(collections) {
  ...
  async function setP2PCredential(email, passwordHash, options = {}) {
    ...
    if (options.fullName) updateDoc.fullName = String(options.fullName).trim();
    if (options.mobile)   updateDoc.mobile   = String(options.mobile).trim();
    if (options.country)  updateDoc.country  = String(options.country).trim();
    if (options.dob)      updateDoc.dob      = String(options.dob).trim();
    if (options.referral) updateDoc.referral = String(options.referral).trim();
  ...
  async function updateP2PCredentialKyc(email, payload = {}) {
    ...
    if (payload.fullName)  patch.fullName  = String(payload.fullName).trim().slice(0, 100);
    if (payload.mobile)    patch.mobile    = String(payload.mobile).trim().slice(0, 20);
    if (payload.address)   patch.address   = String(payload.address).trim().slice(0, 200);
    if (payload.country)   patch.country   = String(payload.country).trim().slice(0, 5);
    if (payload.dob)       patch.dob       = String(payload.dob).trim().slice(0, 12);
    if (status === 'VERIFIED') {
  ...
  async function listOffers(filter = {}) {
    ...
    const rows = await p2pOffers.find(query, { maxTimeMS: 5000 }).limit(filter.limit || 200).sort({ createdAt: -1 }).toArray();
    return rows.map((row) => {
      const mapped = { ...row };
      delete mapped._id;
      return mapped;
  ...
  async function listMyActiveOrders(userId) {
    ...
    const [asSeller, asBuyer, asParticipant] = await Promise.all([
      p2pOrders.find({ sellerUserId: uid, status: statusFilter }, { maxTimeMS: 5000 }).sort({ updatedAt: -1 }).limit(30).toArray(),
      p2pOrders.find({ buyerUserId: uid, status: statusFilter }, { maxTimeMS: 5000 }).sort({ updatedAt: -1 }).limit(30).toArray(),
      p2pOrders.find({ 'participants.id': uid, status: statusFilter }, { maxTimeMS: 5000 }).sort({ updatedAt: -1 }).limit(30).toArray()
    ]);
