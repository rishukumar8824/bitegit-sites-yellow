...
// Hash format v2: "v2:{N}:{salt}:{digest}" — faster N=4096
// Hash format v1: "{salt}:{digest}" — old default N=16384 (backwards compat)
const SCRYPT_N_NEW = 4096; // fast on 0.5 CPU (~50ms), still secure

async function hashPassword(password) {
  const salt = crypto.randomBytes(16).toString('hex');
  const N = SCRYPT_N_NEW;
  const digest = await new Promise((resolve, reject) =>
    crypto.scrypt(password, salt, 64, { N }, (err, key) => err ? reject(err) : resolve(key.toString('hex')))
  );
  return `v2:${N}:${salt}:${digest}`;
}
...
async function verifyPassword(password, storedHash) {
  ...
  if (raw.startsWith('v2:')) {
    ...
    // Old format: {salt}:{digest} — use default N=16384
    const parts = raw.split(':');
