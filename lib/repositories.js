...
function buildCaseInsensitiveExactPattern(value) {
...
function createRepositories(collections) {
  ...
  async function listP2POrderHistory(userId, { limit = 50, offset = 0, username = '', email = '' } = {}) {
    ...
    // Also match by username to handle seed_/usr_ ID format mismatches
    const usernamePattern = buildCaseInsensitiveExactPattern(uname);
    if (usernamePattern) {
      orConditions.push({ sellerUsername: usernamePattern });
      orConditions.push({ buyerUsername: usernamePattern });
      orConditions.push({ 'participants.username': usernamePattern });
    }
    const emailPattern = buildCaseInsensitiveExactPattern(normalizedEmail);
    if (emailPattern) {
      orConditions.push({ sellerEmail: emailPattern });
      orConditions.push({ buyerEmail: emailPattern });
      orConditions.push({ 'participants.email': emailPattern });
    }
    const query = { $or: orConditions };
    const [rows, total] = await Promise.all([
      p2pOrders.find(query).sort({ createdAt: -1 }).skip(safeOffset).limit(safeLimit).toArray(),
      p2pOrders.countDocuments(query)
    ]);
