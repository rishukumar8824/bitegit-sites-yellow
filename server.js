...
// ── Resend Inbound Email Webhook — receives email replies from users ─────────
...
app.post('/api/email/inbound', async (req, res) => {
  try {
    ...
    // e.g. { type: "email.received", data: { from, subject, text, html, ... } }
    const emailData = payload.data || payload;
    const from      = String(emailData.from || emailData.sender || payload.from || payload.sender || '');
    const subject   = String(emailData.subject || payload.subject || '(no subject)').slice(0, 120);
    let htmlBody    = String(emailData.html || payload.html || '').slice(0, 500000);
    const body      = String(emailData.text || payload.text || '').replace(/<[^>]+>/g, '').trim().slice(0, 4000)
                   || htmlBody.replace(/<[^>]+>/g, '').trim().slice(0, 4000);
    const fromEmail = (from.match(/<([^>]+)>/) || [, from])[1] || from;
    const fromName  = (from.match(/^([^<]+)</) || [,''])[1].trim() || fromEmail;

    // Parse attachments (images + files) from Resend payload
    // Gmail inline images come as attachments with inline:true and a contentId (cid:...)
    ...
    // Replace CID references in HTML body with base64 data URLs for inline images
    // Gmail sends: <img src="cid:image001.png@..."> and the image is in attachments
    if (htmlBody && attachments.length > 0) {
      attachments.forEach(att => {
        if (att.contentId && att.content && /^image\//i.test(att.contentType)) {
    ...
    console.log('[inbound-email] received — from:', fromEmail, '| subject:', subject, '| attachments:', attachments.length, '| htmlBody len:', htmlBody.length);
    if (!fromEmail) return res.status(200).json({ ok: true });
    // Save to support_messages — this is what Email Center → Inbox reads
    const _mc = getMongoClient ? getMongoClient() : null;
    if (_mc) {
      await _mc.db().collection('support_messages').insertOne({
        userEmail: fromEmail,
        userName: fromName,
        subject,
        body,
        htmlBody,      // ← CID refs replaced with base64 data URLs
        attachments,   // ← images & files saved here
        source: 'email_reply',
        read: false,
        replies: [],
        createdAt: new Date()
      });
      console.log('[inbound-email] saved to support_messages — from:', fromEmail, '| attachments:', attachments.length);
    } else {
      console.error('[inbound-email] getMongoClient not available!');
    }
...
app.post('/api/support/chat', async (req, res) => {
  try {
    ...
    // Save to DB — use getCollections() directly for resilience (works even if adminStore is initializing)
    let savedId = ticketData.id;
    try {
      const _cols = getCollections ? getCollections() : null;
      const _col = _cols ? _cols.adminSupportTickets : null;
      if (_col) {
        await _col.insertOne(ticketData);
      } else if (adminStore && typeof adminStore.createSupportTicket === 'function') {
        const saved = await adminStore.createSupportTicket(ticketData);
        if (saved && saved.id) savedId = saved.id;
      }
    } catch (_saveErr) {
      console.error('[support/chat] Failed to save ticket:', _saveErr?.message);
    }
    // Notify admin via SSE
    broadcastAdminSupportEvent({
      ticketId: savedId,
